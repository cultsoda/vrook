...
export default function InfluencerDetailPage() {
  ...
  const handleProductClick = (productType: string) => {
    ...
    // 구글 애널리틱스 이벤트 트래킹 (선택사항)
    if (typeof window !== 'undefined' && (window as any).gtag) {
      (window as any).gtag('event', 'product_click', {
        event_category: 'engagement',
        event_label: `${influencer.name}-${productType}`,
        value: 1
      })
    }
  ...
  const handlePackagePurchase = (packageId?: string) => {
    ...
    // 구글 애널리틱스 이벤트 트래킹
    if (typeof window !== 'undefined' && (window as any).gtag) {
      (window as any).gtag('event', 'package_purchase_click', {
        event_category: 'conversion',
        event_label: `${influencer.name}-${packageId || 'default'}`,
        value: 1
      })
    }
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <link rel="preload" href={`/og-${influencer.id}.jpg`} as="image" />
